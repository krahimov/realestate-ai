...
server.tool("search_applications", "Search Toronto development applications by street name, street number, application type, status, ward number, or date range. Returns up to 50 matching results.", {
    streetName: z
        .string()
        .optional()
    ...
    applicationType: z
        .enum(["OZ", "SA", "CD", "SB", "PL"])
        .optional()
        .describe("Application type: OZ=Zoning, SA=Site Plan, CD=Condominium, SB=Subdivision, PL=Planning"),
    status: z
        .string()
        .optional()
        .describe("Application status (e.g. Under Review, Closed, OMB Appeal, NOAC Issued, Council Approved)"),
...
}, async ({ streetName, streetNum, applicationType, status, wardNumber, dateFrom, dateTo, }) => {
    const conditions = [];
    if (streetName)
        conditions.push(ilike(developmentApplications.streetName, `%${streetName}%`));
    if (streetNum)
        conditions.push(eq(developmentApplications.streetNum, streetNum));
    if (applicationType)
        conditions.push(eq(developmentApplications.applicationType, applicationType));
    if (status)
...
server.tool("get_application_details", "Get full details of a specific Toronto development application by its database ID or application number. Returns all fields including contact info, meeting details, and URLs.", {
    ...
    if (applicationNumber)
        conditions.push(eq(developmentApplications.applicationNumber, applicationNumber));
...
server.tool("get_application_stats", "Get aggregate statistics about Toronto development applications. Groups and counts applications by type, status, or ward. Use for 'how many?' questions or summary requests.", {
    groupBy: z
...
server.tool("search_by_description", "Search Toronto development applications by keywords in their description text. Use for queries like 'find residential towers', 'condo projects', 'mixed-use developments'.", {
    keyword: z
        .string()
